...
import { Search, MoreHorizontal, Eye, EyeOff, Copy, Edit, Trash2, User, Lock, Globe } from "lucide-react"
...
export function PasswordList() {
  ...
  const handleMasterPasswordSubmit = async (masterPassword: string) => {
    ...
    try {
      const decrypted = await decryptPassword(pendingAction.encryptedPassword, masterPassword)
  ...
  const requestMasterPassword = (type: "view" | "copy", passwordId: string, encryptedPassword: string) => {
    setPendingAction({ type, passwordId, encryptedPassword })
    setShowMasterPasswordModal(true)
  }

  const togglePasswordVisibility = (passwordId: string, encryptedPassword: string) => {
          ...
          {filteredPasswords.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-muted-foreground">
                {passwords.length === 0 ? "No passwords saved yet" : "No passwords match your search"}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredPasswords.map((password) => {
                ...
                return (
                  <Card key={password._id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg mb-1">{password.title}</h3>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => deletePassword(password._id)} className="text-red-600">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex space-x-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => togglePasswordVisibility(password._id, password.encryptedPassword)}
                              className="h-8 w-8 p-0"
                        ...
                        {decryptedPassword && (
                          <PasswordStrengthIndicator password={decryptedPassword} className="mt-2" />
                        )}
